...
function TpTo(cframe)
    if getRoot(LP.Character) then
        local dis=(getRoot(LP.Character).Position - cframe.Position).Magnitude
        if dis > 50 then
            game:GetService("TweenService"):Create(getRoot(LP.Character), TweenInfo.new(dis/75), {CFrame = cframe}):Play()
        else
            getRoot(LP.Character).CFrame = cframe
        end
    end
end
...
local FixSec = HomeTab:CreateSection({title="Fix"})
FixSec:CreateButton({title="Fix Float",callback=function()
    RemoveFloat()
end})

local ConfigSec = HomeTab:CreateSection({title="Config"})
local selectwep=ConfigSec:CreateDropdown({title="Select Weapon",list=getTools(),default=getgenv().Config.Weapon or "Soul Reaper",multi=false,callback=function(v)
	getgenv().Config.Weapon=v
 end})

ConfigSec:CreateButton({title="Refresh Weapons",callback=function()
selectwep:New(getTools())
end})
ConfigSec:CreateDropdown({title="Use Skill",list={"Z","X","C","V","F"},default=getgenv().Config.UseSkill or {"Z","X","C","V"},multi=true,callback=function(v)
	getgenv().Config.UseSkill=v
end})


local MobSec = HomeTab:CreateSection({title="Mob"})
MobSec:CreateDropdown({title="Select Mob",list=allmobs,default=getgenv().Config.Mob or "",multi=false,callback=function(v)
	getgenv().Config.Mob=v
end})
MobSec:CreateToggle({title="Auto Farm Mob",default=getgenv().Config.AutoFarmMob,callback=function(v)
	getgenv().Config.AutoFarmMob= v
end})

local BossesSec=HomeTab:CreateSection({title="Bosses"})
BossesSec:CreateDropdown({title="Select Boss",list=allbosses,default=getgenv().Config.Bosses or {},multi=true,callback=function(v)
	getgenv().Config.Bosses=v
end})
BossesSec:CreateToggle({title="Auto Farm Boss",default=getgenv().Config.AutoFarmBoss,callback=function(v)
	getgenv().Config.AutoFarmBoss= v
end})

local NearestSec=HomeTab:CreateSection({title="Nearest"})
NearestSec:CreateSlider({title="Distance Farm",Min=100,Precise=getgenv().Config.DistanceFarm or 500,Max=2000,callback=function(v)
	getgenv().Config.DistanceFarm=v
end})
NearestSec:CreateToggle({title="Auto Farm Nearest",default=getgenv().Config.AutoFarmNearest,callback=function(v)
...
EspSec:CreateToggle({title="Player Name ESP",default=getgenv().Config.ShowBossESP,callback=function(v)
...
game:GetService('RunService').Stepped:Connect(function()
	if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild('HumanoidRootPart') and (getgenv().Config.AutoFarmBoss) then
