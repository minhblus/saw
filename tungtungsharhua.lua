...
function UI:CreateWindow(options)
	...
	local function CreateSection(page,optionssec)
		...
		function secfunc:CreateDropdown(dropoptions)
            ...
            show_6.TextTruncate=Enum.TextTruncate.AtEnd
...
repeat 
	...
	wait(.5) 
until not game:GetService("Players").LocalPlayer:GetAttribute("LoadingScreenActive") and game:GetService("Players").LocalPlayer:GetAttribute("LoadingScreenDone")
...
local seedprice={}
local allseed={}
local SeedShopPath=game:GetService("ReplicatedStorage").StockValues.SeedShop.Items
local GearShopPath=game:GetService("ReplicatedStorage").StockValues.GearShop.Items
local Networking = require(game:GetService("ReplicatedStorage").SharedModules.Networking)

for i,v in pairs(game:GetService("ReplicatedStorage").StockValues.SeedShop.Items:GetChildren()) do
    table.insert(allseedsshop,v.Name)
end

local seeddata = require(game:GetService("ReplicatedStorage").SharedModules.SeedData)
for i,v in pairs(seeddata) do
	if v.PurchasePrice then
		seedprice[v.SeedName]=v.PurchasePrice
	end
	table.insert( allseed, v.SeedName )
end

local allgearshop={}
for i,v in pairs(game:GetService("ReplicatedStorage").StockValues.GearShop.Items:GetChildren()) do
    table.insert(allgearshop,v.Name)
end

local allgear={}
local gearprice={}
local geardata = require(game:GetService("ReplicatedStorage").SharedModules.GearShopData)
for i,v in pairs(geardata.Data) do

	if v.Cost then
		gearprice[v.ItemName]=v.Cost
		print(v.ItemName,v.Cost)
	end
	table.insert( allgear, v.ItemName )
end

local garden

repeat task.wait() 
	for i,v in pairs(workspace.Gardens:GetChildren()) do
		if v:GetAttribute("OwnerUserId") == game:GetService("Players").LocalPlayer.UserId then
			garden = v
		end
	end
...
function GetSeedBackpack()
	local seedout={}

	for i,v in pairs(game:GetService("Players").LocalPlayer.Backpack:GetChildren()) do
		if v:GetAttribute("MainCategory") and v:GetAttribute("MainCategory") == "Seed" then

			table.insert( seedout, v)
		end
	end
	return seedout
end
...
local Visual = garden.Visual

local Columns = {
    Visual.PlantAreaColumn1,
    Visual.PlantAreaColumn2
}

local function PlaceSeedAtPosition(position,v)
	Networking.Plant.PlantSeed:Fire(position, v:GetAttribute("SeedTool"), v)
end

local function PlantEntireGarden(seed)
	for i,v in pairs(seed) do
		for _, column in ipairs(Columns) do
			local size = column.Size
			local cframe = column.CFrame
			...
			for x = -size.X/2, size.X/2, stepX do
				for z = -size.Z/2, size.Z/2, stepZ do
					
					local offset = (right * x) + (look * z)
					local calculatedPos = (cframe + offset).Position
					
					local targetPosition = Vector3.new(calculatedPos.X, 142.3554229736328, calculatedPos.Z)
					PlaceSeedAtPosition(targetPosition,v)
...
local petmodule = require(game:GetService("ReplicatedStorage").SharedData.PetData)

for i,v in pairs(petmodule) do
	if type(v)=="table" then
		if v.SpawnChance and v.BasePrice then
			table.insert( petdata, i )
		end
	end
end
...
for i,v in pairs(game:GetService("ReplicatedStorage").SharedModules.MutationData:GetChildren()) do
	table.insert( allmutations, v.Name )
end
...
local function CheckBuyPet()
	for i,v in pairs(workspace.Map.WildPetRef:GetChildren()) do
		if table.find(getgenv().Config.ChoosePets, v:GetAttribute("PetName")) and v:GetAttribute("OwnerUserId") ~= plr.UserId and plr.leaderstats.Sheckles.Value >= v:GetAttribute("Price") then
			return v
		end
	end
	return false
...
local function WaitBuyPet()
	for i,v in pairs(workspace.Map.WildPetRef:GetChildren()) do
		if table.find(getgenv().Config.ChoosePets, v:GetAttribute("PetName")) and v:GetAttribute("OwnerUserId") == plr.UserId then
			return true
		end
	end
	return false
...
local function FormatPets(listpet)
    local counts = {}

    for _, petData in ipairs(listpet) do
        local petName = petData[1]
        local rarity = petData[2]
		local price= petData[3]

        local key = petName .. "|" .. rarity .. "|" .. price
    ...
    return rewards ~= "" and rewards or "No pets found"
end

local function Webhook(listpet)
	if not getgenv().Config.WebhookLink then
		return
	end
    local rewards = FormatPets(listpet)
    ...
    request({
        Url = getgenv().Config.WebhookLink,
        Method = "POST",
        Headers = {
...
local function WebhookBuy(namepet)
	if not getgenv().Config.WebhookLink then
		return
	end
    local rewards = namepet
...
local function rqpet()
    local out = {}

    for _, v in ipairs(workspace.Map.WildPetRef:GetChildren()) do
        table.insert(out, {
            v:GetAttribute("PetName"),
            v:GetAttribute("Rarity"),
			v:GetAttribute("Price")
        })
    end
...
local dataplr = PlayerStateClient:GetLocalReplica()
...
SeedSec:CreateDropdown({
    Name = "Choose Seeds",
    List = allseedsshop,
    Default = getgenv().Config.ChooseSeeds,
    Multi = true,
    Callback = function(selectedList)
        getgenv().Config.ChooseSeeds=selectedList
    end
})

SeedSec:CreateToggle({
	Name="Auto Buy",
	Default=getgenv().Config.AutoBuySeed or false,
	Callback = function(value)
		getgenv().Config.AutoBuySeed=value
		task.spawn(function()
			while getgenv().Config.AutoBuySeed do
				task.wait(1)
				for i,v in pairs(getgenv().Config.ChooseSeeds) do
					if SeedShopPath[v].Value > 0 then
						if game:GetService("Players").LocalPlayer.leaderstats.Sheckles.Value >= seedprice[v] and (not dataplr.Data.PurchasedThisRestock.Seeds[v] or dataplr.Data.PurchasedThisRestock.Seeds[v] < SeedShopPath[v].Value) then
							Networking.SeedShop.PurchaseSeed:Fire(v)
						end
					end
				end
			end
...
local GearSec=Sawhub.Home:CreateSection({Name="Gears"})

GearSec:CreateDropdown({
    Name = "Choose Gears",
    List = allgearshop,
    Default = getgenv().Config.ChooseGear,
    Multi = true,
    Callback = function(selectedList)
        getgenv().Config.ChooseGear=selectedList
    end
})

GearSec:CreateToggle({
	Name="Auto Buy",
	Default=getgenv().Config.AutoBuyGear or false,
	Callback = function(value)
		getgenv().Config.AutoBuyGear=value
		task.spawn(function()
			while getgenv().Config.AutoBuyGear do
				task.wait(1)
				for i,v in pairs(getgenv().Config.ChooseGear) do
					if GearShopPath[v].Value > 0 then
						print(dataplr.Data.PurchasedThisRestock.Gears[v],gearprice[v])
						if game:GetService("Players").LocalPlayer.leaderstats.Sheckles.Value >= gearprice[v] and (not dataplr.Data.PurchasedThisRestock.Gears[v] or dataplr.Data.PurchasedThisRestock.Gears[v] < GearShopPath[v].Value) then
							Networking.GearShop.PurchaseGear:Fire(v)
						end
					end
				end
			end
...
HarvestSec:CreateDropdown({
    Name = "Choose Seeds",
    List = allseed,
    Default = getgenv().Config.ChooseSeedsHarvest or {},
    Multi = true,
    Callback = function(selectedList)
        getgenv().Config.ChooseSeedsHarvest=selectedList
    end
})

HarvestSec:CreateDropdown({
    Name = "Mutations Harvest",
    List = allmutations,
    Default = getgenv().Config.MutationHarvest or {},
    Multi = true,
    Callback = function(selectedList)
        getgenv().Config.MutationHarvest=selectedList
    end
})
HarvestSec:CreateToggle({
	Name="Auto Harvest",
	Default=getgenv().Config.AutoHarvest or false,
	Callback = function(value)
...
HarvestSec:CreateToggle({
	Name="Auto Sell (max bp)",
	Default=getgenv().Config.AutoSell or false,
	Callback = function(value)
		getgenv().Config.AutoSell=value
		task.spawn(function()
			while getgenv().Config.AutoSell and task.wait(.1) do
				if countfruit() >= 98 then
...
HarvestSec:CreateToggle({
	...
	Default=getgenv().Config.AutoSellCooldown or false,
	Callback = function(value)
		getgenv().Config.AutoSellCooldown=value
		task.spawn(function()
			while getgenv().Config.AutoSellCooldown and task.wait(.1) do
				if tick()-ticksell >= getgenv().Config.DelaySell then
					ticksell=tick()
					Networking.NPCS.SellAll:Fire()
				end
			end
...
HarvestSec:CreateSlider({
    Name = "Delay Sell (s)",
    Min = 30,
    Max = 600,
    Default = getgenv().Config.DelaySell or 60,
    Callback = function(value)
        getgenv().Config.DelaySell=value
    end
})

local GardenSec=Sawhub.Home:CreateSection({Name="Garden"})

GardenSec:CreateDropdown({
    Name = "Choose Seeds",
    List = allseed,
    Default = getgenv().Config.ChooseSeedsPlant or {},
    Multi = true,
    Callback = function(selectedList)
        getgenv().Config.ChooseSeedsPlant=selectedList
    end
})

GardenSec:CreateToggle({
	Name="Auto Plant",
	Default=getgenv().Config.AutoPlant or false,
	Callback = function(value)
		getgenv().Config.AutoPlant=value
		task.spawn(function()
			while getgenv().Config.AutoPlant and task.wait() do
				PlantEntireGarden(GetSeedBackpack())
			end
		end)
	end
}) 

local PetSec=Sawhub.Home:CreateSection({Name="Pets"})

PetSec:CreateDropdown({
    Name = "Choose Pets",
    List = petdata,
    Default = getgenv().Config.ChoosePets or {},
    Multi = true,
    Callback = function(selectedList)
        getgenv().Config.ChoosePets=selectedList
    end
})

PetSec:CreateToggle({
	Name="Auto Buy",
	Default=getgenv().Config.AutoBuyPet or false,
	Callback = function(value)
		getgenv().Config.AutoBuyPet=value
	end
}) 

PetSec:CreateToggle({
	Name="Auto Rejoin Buy",
	Default=getgenv().Config.AutoRejoinBuy or false,
	Callback = function(value)
		getgenv().Config.AutoRejoinBuy=value
		task.spawn(function()
			while task.wait() and getgenv().Config.AutoRejoinBuy do 
				if not CheckBuyPet() and not WaitBuyPet() then
					CreateNotify("Rejoin After 5s",3)
					task.wait((getgenv().Config.RejoinDelay or 2) + 5)
...
PetSec:CreateSlider({Name="Delay Rejoin (s)",Min=0,Default=getgenv().Config.RejoinDelay or 2,Max=10,Precise=getgenv().Config.RejoinDelay or 2,Callback=function(v)
    getgenv().Config.RejoinDelay=v
end})

local ShovelSec=Sawhub.Home:CreateSection({Name="Shovel"})

ShovelSec:CreateDropdown({
    Name = "Choose Seeds",
    List = allseed,
    Default = getgenv().Config.ChooseSeedsShovel or {},
    Multi = true,
    Callback = function(selectedList)
        getgenv().Config.ChooseSeedsShovel=selectedList
    end
})

ShovelSec:CreateButton({
	Name="Shovel",
	Callback = function()
		task.spawn(function()
			for i,v in pairs(garden.Plants:GetChildren()) do
				local Shovel = plr.Backpack:FindFirstChild("Shovel") or plr.Character:FindFirstChild("Shovel")
				if Shovel and table.find(getgenv().Config.ChooseSeedsShovel,v:GetAttribute("SeedName"))then

					Networking.Shovel.UseShovel:Fire(v.Name, "", "Shovel", Shovel)
					task.wait(.1)
				end
			end
		end)
...
ConfigSec:CreateSlider({Name="Set Fps",Min=2,Default=getgenv().Config.FpsCap or 120,Max=9999,Callback=function(v)
...
ConfigSec:CreateToggle({Name="White Screen",Default=getgenv().Config.WhiteScreen,Callback=function(v)
...
ConfigSec:CreateButton({Name="Copy Config",Callback=function()
    local save_configs = "getgenv().Config = {\n    "
    for i, v in pairs(getgenv().Config) do
		if (type(v)=="bool" and not v) or (type(v)=="table" and #v==0) then
			continue
		end
...
task.spawn(function()
	while task.wait() do
		if getgenv().Config.AutoBuyPet and CheckBuyPet() then
			TweenFloat(true)
			TaskFarm="AutoBuyPet"
		elseif getgenv().Config.AutoHarvest then
			TweenFloat(true)
			TaskFarm="AutoHarvest"
		else
			TweenFloat(false)
		end
	end
...
task.spawn(function()
	while task.wait() do
		if getgenv().Config.AutoBuyPet and TaskFarm=="AutoBuyPet" then
			local pet = CheckBuyPet()
			if pet then
				plr.Character.HumanoidRootPart.CFrame=pet.CFrame
				Networking.Pets.WildPetTame:Fire(pet)
				if tick()-ticdelay >= 1 then
					ticdelay=tick()
					WebhookBuy(pet:GetAttribute("PetName") .. " | " .. pet:GetAttribute("Rarity") .. " | ".. pet:GetAttribute("Price"))
				end
			end
		end
	end
...
task.spawn(function()
	while task.wait(.01) do
		if getgenv().Config.AutoHarvest and TaskFarm=="AutoHarvest" then
			plr.Character.HumanoidRootPart.CFrame=garden.Visual.PRIM.CFrame
			for i,v in pairs(garden.Plants:GetChildren()) do
				HarvestFruit(v)
				if v:FindFirstChild("Fruits") then
					for _,cc in pairs(v.Fruits:GetChildren()) do
